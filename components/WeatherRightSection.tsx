...
export default function WeatherRightSection({ weatherData, loading }: Props) {
  const [detailListSlicePrismic, setDetailListSlicePrismic] = useState<any>(null);
  const [forecastIcons, setForecastIcons] = useState<any[]>([]);
  ...
  useEffect(() => {
    const fetchPrismicContent = async () => {
      try {
        ...
        const detailList = slices.find((s: any) => s.slice_type === "weather_detail_list");
        setDetailListSlicePrismic(detailList);

        // Weather Forecast slice (for bottom cards icons)
        const forecast = slices.find((s: any) => s.slice_type === "weather_forecast");
        if (forecast) {
          const primary = forecast.primary as any;
          const forecastItems = [
    ...
    <div className="flex-1 flex flex-col gap-2 md:gap-4 lg:mt-0 ">
      ...
      <div className="bg-[#1E1E29] rounded-xl pt-4 pb-4 px-4 md:p-6 h-[350px] xlg:max-h-[calc(50vh-48px)] xlg:overflow-hidden">
        ...
        <div className="flex flex-col gap-3 xlg:gap-2">
          {detailListSlicePrismic?.primary?.weather_items?.map((item: any, idx: number) => {
            // Map API values based on label
            let value = "";
            switch (item.label) {
              case "Feels Like":
                value = weatherData ? `${Math.round(weatherData.feels_like)}°C` : "—";
                break;
              case "Probability of Rain":
                value = weatherData ? `${weatherData.rain_probability}%` : "—";
                break;
              case "Wind Speed":
      ...
      <div className="bg-[#1E1E29] rounded-xl p-3 sm:p-6 xlg:h-[240px] overflow-hidden">
        ...
        <div className="flex justify-between gap-4">
          {weatherData &&
            Object.entries(weatherData.next_days_temperature).map(([date, info], idx) => {
  ...
  <span className="block sm:hidden">{info.day.slice(0, 3)}</span>
