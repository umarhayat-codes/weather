...
import { NextDaysTemperature, OpenWeatherForecastItem } from "@/types/weather";
...
export async function GET(req: NextRequest) {
  try {
    ...
    const currentRes = await axios.get(`${BASE_URL}/data/2.5/weather`, {
      params: { lat, lon, units: "metric", appid: API_KEY },
    });
    const current = currentRes.data;

    // Step 3 → Forecast 3-hourly
    const forecastRes = await axios.get(`${BASE_URL}/data/2.5/forecast`, {
      params: { lat, lon, units: "metric", appid: API_KEY },
    });
    ...
    try {
      const uvRes = await axios.get(`${BASE_URL}/data/2.5/uvi`, {
    ...
    forecast.list.forEach((item: OpenWeatherForecastItem) => {
      ...
      if (!allDates[dateTxt]) {
        ...
        allDates[dateTxt].min = Math.min(
          allDates[dateTxt].min,
          item.main.temp_min
        );
        allDates[dateTxt].max = Math.max(
          allDates[dateTxt].max,
          item.main.temp_max
        );
    ...
    const nextDays: NextDaysTemperature = Object.fromEntries(
      Object.entries(allDates).slice(0, 5)
    );

    const today_condition = current.weather?.[0]?.main;
